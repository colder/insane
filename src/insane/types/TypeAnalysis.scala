  ...
  class TypeAnalysisPhase extends SubPhase {
    ...
    def analyze(f: AbsFunction) {
      ...
      def methodCall(call: CFG.AssignApplyMeth, obj: CFG.Ref, oset: ObjectSet,  ms: Symbol) {
        ...
        val matches = getMatchingMethods(ms.name, methodType, oset, call.pos, call.isDynamic).map(_._1)
        ...
        if (settings.dumpCallStats) {
          ...
          var allMatches = getMatchingMethods(ms.name, methodType, ObjectSet.subtypesOf(ms.owner.tpe), call.pos, call.isDynamic).size
