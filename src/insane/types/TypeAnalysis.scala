  ...
  class TypeAnalysisPhase extends SubPhase {
    ...
    def analyze(f: AbsFunction) {
      ...
      def methodCall(call: CFG.AssignApplyMeth, obj: CFG.Ref, info: TypeInfo,  ms: Symbol) {
        ...
        val callSig = TypeSignature.fromDeclaration(ms).copy(rec = SigEntry.fromTypeInfo(info))

        val matches = getMatchingMethods(ms, call.style, callSig).map(_.sym)

        if (call.style == CFG.DynamicCall && matches.isEmpty) {
        ...
        if (settings.dumpCallStats) {
          ...
          var allMatches = getMatchingMethods(ms, call.style, TypeSignature.fromDeclaration(ms)).size
