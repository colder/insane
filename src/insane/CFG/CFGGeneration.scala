        ...
        tree match {
          case ArrayValue(tpt, elems) =>
            ...
            for ((elem, i) <- elems.zipWithIndex) {
              Emit.statement(new CFG.AssignApplyMeth(unusedVariable() setTree elem, to, definitions.Array_update, List(new CFG.LongLit(i) setTree elem, convertTmpExpr(elem, "arrelem")), false, Set(), Set()) setTree elem)
            }
          ...
          case a @ ExNew(sym, args) =>
            ...
            Emit.statement(new CFG.AssignApplyMeth(unusedVariable() setTree a, ref, sym, args.map(convertTmpExpr(_, "arg")), false, Set(), Set()) setTree a)
          ...
          case ad @ ApplyDynamic(o, args) =>
            ...
            Emit.statement(new CFG.AssignApplyMeth(to, obj, ad.symbol, args.map(convertTmpExpr(_, "arg")), true, Set(), Set()) setTree ad)
          ...
          case a @ Apply(s @ Select(o, meth), args) =>
            ...
            if (s.symbol == definitions.Boolean_or || s.symbol == definitions.Boolean_and) {
              ...
              val obj = convertTmpExpr(o, "obj")
              Emit.statement(new CFG.AssignApplyMeth(to, obj, s.symbol, args.map(convertTmpExpr(_, "arg")), false, Set(), Set()) setTree a)
            }
