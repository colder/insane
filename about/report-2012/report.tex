...
Algorithm~\ref{algo:pt:methcall}. Most dependencies are also described in
pseudo-code in their respective figure.
...
\begin{algorithmic}[1]
\Function{inlineByCFGs}{$env, targetCFGs$}
    \If{targetCFGs empty}
        \State \Return env
    \Else
...
\begin{algorithmic}[1]
\Function{inlineByEffects}{$env, targetCFGs$}
    \If{effectsStore contains (meth, Precise, callArgsTypes)}
        \State \Return effectsStore get (meth, Precise, callArgsTypes)
    \Else
        \State \Return analyze(meth, Precise, callArgsTypes)
