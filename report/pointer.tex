...
each to each node they point to, drawn in blue. Return nodes are drawn using
double circle around nodes. Inside edges are drawn using a full edge, while
...
\begin{itemize}
    ...
    loop yields multiple objects at the same allocation site\cite{DBLP:conf/pldi/ChaseWZ90}
.
\end{itemize}
...
describes in full detail how we resolve load nodes when inlining the graph of
