...
program point. Upon method calls, we inline the graph of the callee into the
...
the graph representation developed in \cite{Salcianu2006}. We can point out two
...
\end{lstlisting}
we know that \lstinline{arg.f} points to \lstinline{arg}, and thus we do not need to
...
\end{figure}
At the end of the \lstinline{test} method, \lstinline{a.next} either points to \lstinline{a} or to
the old value, represented by the load node, which depends on the actual object
passed to \lstinline{test()}.
...
after the branch, the value of \lstinline{a.f} is either 2 or 3, but never 1. We
...
Algorithm~\ref{algo:pt:safelnode} describes how we defined $safeLNode$ for such purpose.
...
First of all, we argue that the graphs have a limited number of nodes. We have two
...
$safeCompose$ operation only generates a bounded number of composed program points. The
...
The argument above is however not sufficient to claim termination, as we still have to show
...
inlining, would be required to produce a formal proof, but is beyond
the scope of this work.

\section{Loop/Function Equivalence}
Even though Scala can be used as a one-to-one substitute for an imperative language, its
...
recursive loops and their corresponding functional version are equivalently represented
...
We have thus informally established that both fix-points represent the
same effects. And thus, our analysis has the same precision for either programming style.
...
warnings in case a read or write operation is performed from a \emph{null}
node.
