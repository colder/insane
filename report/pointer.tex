...
We now describe how we determine the \emph{singleton} flag for newly allocated
objects, e.g. INodes, as it is the only node kind for which it is not set to a
fixed value. Without any kind of recency abstraction, we wouldn't be able to
perform strong updates on inside nodes. The rationale is that since INodes are
identified using program points (allocation sites), one INode potentially
represents many objects. As we previously saw, allowing strong update is key in
providing a precise analysis.

There are two ways of generating inside nodes corresponding to multiple
objects: loops and function calls. We provide here the general idea that will
be able to handle both cases soundly. A refinement of this technique in the
presence of function calls will be described in
Section~\ref{sec:pt:allocinline}.

The idea is simple, when encountering an allocation \verb/r = new T/ at program
point $@p$, and before actually including the inside node into the environment,
we look whether it is already present. In such case, we are in the presence of
a loop, and this inside node will have its \emph{singleton} flag set to
\emph{false}. Otherwise, we add the inside node with \emph{singleton} equal to
\emph{true}.
...
information encompass information about every variables.
...
\begin{minipage}[tr]{0.5\linewidth}
    ...
    \begin{tabular}{c}
        ...
        \end{tabular} \\
    \subfloat[Result]{\includegraphics[scale=0.6]{images/pt_join1res}}
    \end{tabular}
\end{minipage}
...
\begin{minipage}[tr]{0.5\linewidth}
    ...
    \begin{tabular}{c}
        ...
        \end{tabular} \\
    \subfloat[Result]{\includegraphics[scale=0.6]{images/pt_join2res}}
    \end{tabular}
\end{minipage}
...
\begin{minipage}[tr]{0.5\linewidth}
    ...
    \begin{tabular}{c}
        ...
        \end{tabular} \\
    \subfloat[Result]{\includegraphics[scale=0.6]{images/pt_join3res}}
    \end{tabular}
\end{minipage}
...
\subsection{Inlining of Allocation Sites}
...
For instance, Figure~\ref{pt:equi:code} provides two equivalent implementations
